            ...
            start: function (tag, attrs, unary) {
                ...
                if (tag === 'img') {
                    ...
                    if (tmpSrc === '') {
                        ...
                        lastFragment = tmpAttrsTxt.length === 0 ? '<img></img>' : '<img' + tmpAttrsTxt + '></img>'
                    }
                } else if (tag === 'a') {
                    let tmpAttrsTxt = ''
                    for (let i = 0; i < attrs.length; i++) {
...
function getContent(htmlContent) {
    try {
        ...
        let a = parseHTML(tmpHtml)
        return a.replace(/<br><\/br><br><\/br>/g, '<br><\/br>').replaceAll("\n","").replaceAll(/> +</ig, "><")
    } catch (e) {
        console.log('Error:', e)
        return htmlContent
    }
...
chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
    ...
    tmpGlobalContent = tmpContent + `<br/><a href="${document.location.href}">${document.location.href}</a>`
    ...
    result = {
        ...
        styleFileContent: styleFile == null ? '' : styleFile,
